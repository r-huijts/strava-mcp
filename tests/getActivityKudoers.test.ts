...
describe("getActivityKudoers client", () => {
    ...
    it("rejects invalid response data", async () => {
        vi.spyOn(stravaApi, "get").mockResolvedValue({ data: [{ firstname: "Missing required fields" }] } as any);
...
describe("get-activity-kudoers tool", () => {
    ...
    it("returns kudoer summaries and raw data", async () => {
        ...
        expect(result.content[0].text).toContain("Alice R (ID: 123)");
        expect(result.content[0].text).toContain("Bob S (ID: 456)");
